...
@frappe.whitelist()
def get_spa_invoices_print(from_date, to_date=None, invoices=None):
	"""Printable version of the Invoices tab's currently filtered list -
	re-derives nothing server-side (unlike the old per-period Transactions
	report this replaces); it just renders whatever get_spa_invoices()
	already returned to the caller (and is showing on screen) into a
	print-ready HTML document, the same pattern cashier_portal.py's
	get_daily_transactions_print() uses."""
	if isinstance(invoices, str):
		invoices = json.loads(invoices)

	date_label = str(from_date) if not to_date or to_date == from_date else f"{from_date} to {to_date}"
	currency = frappe.defaults.get_global_default("currency")
	total = sum(flt(inv.get("grand_total")) for inv in (invoices or []))
	...
	rows = "".join(
		...
		<tr>
			<td>{i + 1}</td>
			<td>{inv.get('name')}</td>
			<td>{inv.get('posting_date')}</td>
			<td>{inv.get('patient_name') or inv.get('customer_name') or ''}</td>
			<td>{inv.get('status', '')}</td>
			<td style="text-align:right">{frappe.utils.fmt_money(inv.get('grand_total'), currency=currency)}</td>
		</tr>
		"""
		for i, inv in enumerate(invoices or [])
	)

	html = f"""
		<html>
		<head>
			<title>Spa Invoices - {date_label}</title>
			<style>
